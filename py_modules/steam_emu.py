...
def _iter_steam_shortcuts():
    ...
    Hydra's library doesn't track a game's install directory, so this is the
    only reliable way to locate it (Steam's shortcuts.vdf already has the
    resolved path).
    """
...
def get_settings(title: str):
    ini_path = find_ini_by_title(title)
    if not ini_path:
        return None
